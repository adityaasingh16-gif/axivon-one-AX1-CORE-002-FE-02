...
(function (global) {
  ...
  function getUsers() {
    var users = readJSON(localStorage, LS_USERS, []);
    if (!Array.isArray(users)) users = [];
    if (!users.some(function (u) { return u.email === 'owner@axivon.one'; })) {
      users.push({
        id: 'owner_axivon_one', fullName: 'AXIVON ONE Owner', email: 'owner@axivon.one',
        passwordHash: demoHash('Axivon@123'), verified: true, verificationCode: null,
        verificationSentAt: null, resetToken: null, resetExpires: null, createdAt: nowMs()
      });
      saveUsers(users);
    }
    return users;
  ...
  function requireAuth() {
    var s = getSession();
    if (!s) {
      // User-management pages use a dedicated access screen so the
      // employee/admin area is visually distinct from the public login.
      var file = location.pathname.split('/').pop() || 'dashboard.html';
      var userAreaPages = {
        'user-management.html': true,
        'users.html': true,
        'user-details.html': true,
        'user-form.html': true,
        'status.html': true,
        'membership.html': true
      };
      var loginPage = userAreaPages[file] ? 'user-login.html' : 'login.html';
      var destination = file + (location.search || '') + (location.hash || '');
      location.replace(loginPage + '?redirect=' + encodeURIComponent(destination));
      return null;
    }
    return s;
  }
